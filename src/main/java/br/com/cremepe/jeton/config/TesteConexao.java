...
/**
 * Componente de diagnóstico estrutural e auditoria para o banco de dados Jeton.
 * Executa automaticamente na inicialização da aplicação para exibir tabelas,
 * views,
 * metadados, dados amostrais (top 5) e código completo dos gatilhos (triggers).
 */
@Component
public class TesteConexao implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        System.out.println("\n=========================================================================");
        System.out.println("   INICIANDO AUDITORIA E DIAGNÓSTICO PROFUNDO DO BANCO DE DADOS JETON");
        System.out.println("=========================================================================\n");
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            System.out.println("[INFO] SGBD: " + conn.getMetaData().getDatabaseProductVersion());
            System.out.println("-------------------------------------------------------------------------\n");
            ...
            System.err.println("[ERRO CRÍTICO] Falha ao executar diagnóstico no banco de dados: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private void mapearTabelasERestricoes(Connection conn, String catalog) throws SQLException {
        DatabaseMetaData metaData = conn.getMetaData();

        try (ResultSet rsTabelas = metaData.getTables(catalog, null, "%", new String[] { "TABLE" })) {
            while (rsTabelas.next()) {
                ...
                try (ResultSet rsPk = metaData.getPrimaryKeys(catalog, null, nomeTabela)) {
                    ...
                    while (rsPk.next()) {
                        temPk = true;
                        System.out.println("  -> Campo Primário: " + rsPk.getString("COLUMN_NAME") + " [Constraint: "
                ...
                try (ResultSet rsFk = metaData.getImportedKeys(catalog, null, nomeTabela)) {
                    ...
                    while (rsFk.next()) {
                        temFk = true;
                        System.out.println("  -> CONSTRAINT: " + rsFk.getString("FK_NAME") + " | Local: "
    ...
    private void mapearViewsDoSistema(Connection conn, String catalog) throws SQLException {
        DatabaseMetaData metaData = conn.getMetaData();

        System.out.println("#########################################################################");
        System.out.println("           MAPEAMENTO DE VISÕES LOGICAS (VIEWS) DO BANCO DE DADOS");
        System.out.println("#########################################################################\n");

        try (ResultSet rsViews = metaData.getTables(catalog, null, "%", new String[] { "VIEW" })) {
            while (rsViews.next()) {
                ...
                System.out.printf("%-25s | %-15s | %-8s\n", "CAMPO", "TIPO DE DADO", "ACEITA NULO");
                System.out.println("-------------------------------------------------------------------------");
                ...
                System.out.println("-------------------------------------------------------------------------");
                String sqlDef = "SELECT VIEW_DEFINITION FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?";
    ...
    private void exibirDadosAmostra(Connection conn, String nomeAlvo) {
        ...
        try (Statement stmt = conn.createStatement();
            ...
            System.out.println("  [AVISO] Não foi possível extrair amostra de dados: " + e.getMessage());
        }
    }

    private void mapearGatilhosTriggers(Connection conn, String catalog) throws SQLException {
        System.out.println("#########################################################################");
        System.out.println("       GATILHOS (TRIGGERS) ATIVOS DE VALIDAÇÃO FINANCEIRA");
        System.out.println("#########################################################################\n");

        String sql = "SELECT TRIGGER_NAME, EVENT_MANIPULATION, EVENT_OBJECT_TABLE, ACTION_TIMING, ACTION_STATEMENT " +
                "FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_SCHEMA = ? ORDER BY EVENT_OBJECT_TABLE ASC";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    System.out.println("=========================================================================");
                    System.out.println(" TRIGGER: " + rs.getString("TRIGGER_NAME").toUpperCase());
                    System.out.println("=========================================================================");
                    System.out.println(" Tabela Vinculada : " + rs.getString("EVENT_OBJECT_TABLE"));
