...
public class GestaoConselheiroController {
    ...
    public String listar(
        ...
        Page<GestaoConselheiro> pagina = gestaoConselheiroService.listarComPaginacaoEPesquisa(termo, situacao, page, size, sort, dir);
    ...
    public String excluir(@PathVariable("idGestao") Integer idGestao, 
        ...
        try {
            ...
            ra.addFlashAttribute("erro", "Não foi possível remover o vínculo. O conselheiro já possui atividades nesta gestão.");
        }
        return "redirect:/gestao-conselheiros";
    }
    ...
    public String prepararVincularMassa(@PathVariable("idGestao") Integer idGestao, Model model, HttpSession session) {
        ...
        List<Integer> idsComAtividades = selecionadosIds.stream()
                .filter(idPessoa -> atividadeRepository.countByGestaoIdGestaoAndConselheiroIdPessoa(idGestao, idPessoa) > 0)
                .toList();
